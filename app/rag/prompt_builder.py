...
- Cite toujours la source (ex: Jira IH-1, ServiceNow INC001)
- Réponds en français
- Ne spécule pas, ne complète pas avec tes connaissances générales"""


def build_prompt(question: str, chunks: list[SearchResult]) -> list[dict]:
...
def _build_context(chunks: list[SearchResult]) -> str:
    ...
    for i, chunk in enumerate(chunks, 1):
        ...
        parts.append(
            f"[{i}] {source_label} (similarité: {chunk.similarity})\n"
            f"{chunk.content[:500]}"
        )
...
def _source_label(source_type: str, document_id: str) -> str:
    labels = {
        ...
        "servicenow": f"ServiceNow {document_id}",
